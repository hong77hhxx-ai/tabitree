...
export default function Profile({ nickname, avatarUrl, onSave }: ProfileProps) {
  ...
  return (
    <div
      className="flex flex-col h-full bg-[var(--surface-muted)]"
      style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}
    >
      {/* ヘッダー */}
      <div className="bg-[var(--surface)] border-b border-[var(--border-soft)] px-4 pt-5 pb-4 flex-shrink-0">
        <h1 className="text-xl font-bold text-[var(--text-strong)]">プロフィール</h1>
      </div>
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex gap-3">
              <label className={`flex items-center gap-1.5 text-sm font-bold px-4 py-2.5 rounded-2xl border cursor-pointer transition-all active:scale-95 bg-[var(--surface)] border-[var(--border-soft)] text-[var(--text-muted)] shadow-sm ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}>
                <Camera size={16} />
                カメラ
          ...
          {/* 名前 */}
          <div className="bg-[var(--surface)] rounded-2xl p-5 shadow-sm border border-[var(--border-soft)] space-y-3">
            <label className="block text-sm font-bold text-[var(--text-strong)]">ニックネーム</label>
            <input
              type="text"
              value={name}
              ...
              placeholder="例: たろう"
              maxLength={20}
              className="w-full px-4 py-4 rounded-xl border border-[var(--border-soft)] bg-[var(--surface-sunken)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] transition-all text-[var(--text-strong)] text-lg font-bold"
            />
            <p className="text-xs text-[var(--text-muted)]">マップ上で他のメンバーに表示されます</p>
          </div>
